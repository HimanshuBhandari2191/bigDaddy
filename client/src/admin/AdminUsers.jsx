...
const AdminUsers = () => {
  ...
  return (
    <div style={containerStyle}>
      <h2 style={{ color: '#e5e5e5', marginBottom: '20px' }}>User Directory</h2>
          ...
          <tbody>
            {users.map(u => (
              <tr key={u._id} style={rowStyle}>
                ...
                <td style={tdStyle}>
                  <span style={{ background: u.role === 'admin' ? '#f5f5f5' : 'transparent', color: u.role === 'admin' ? '#0a0a0a' : '#f5f5f5', border: u.role === 'admin' ? 'none' : '1px solid rgba(255,255,255,0.25)', padding: '4px 8px', borderRadius: '4px', fontSize: '0.85rem', fontWeight: 'bold' }}>
                    {u.role.toUpperCase()}
                  </span>
                </td>
